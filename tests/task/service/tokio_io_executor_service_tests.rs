 ...
 *
 ******************************************************************************/
//! Tests for [`TokioIoExecutorService`](qubit_concurrent::task::service::TokioIoExecutorService).

use std::io;

use qubit_concurrent::task::{TaskExecutionError, service::TokioIoExecutorService};
...
async fn test_tokio_io_executor_service_shutdown_rejects_new_tasks() {
    ...
    assert!(matches!(
        result,
        Err(qubit_concurrent::task::service::RejectedExecution::Shutdown)
    ));
